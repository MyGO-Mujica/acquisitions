...
export const fetchUserById = async (req, res, next) => {
  try {
    logger.info(`正在获取用户，ID: ${req.params.id}`);
    ...
    logger.error(`通过ID获取用户时发生错误: ${e.message}`);
...
export const updateUserById = async (req, res, next) => {
  try {
    logger.info(`正在更新用户，ID: ${req.params.id}`);
    ...
    logger.error(`更新用户信息时发生错误: ${e.message}`);
...
export const deleteUserById = async (req, res, next) => {
  try {
    logger.info(`正在删除用户，ID: ${req.params.id}`);
    ...
    logger.info(`用户 ${deletedUser.email} 删除成功`);
    res.json({
      message: 'User deleted successfully',
      user: deletedUser,
    });
  } catch (e) {
    logger.error(`删除用户时发生错误: ${e.message}`);
